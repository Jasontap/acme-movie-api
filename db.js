...
const syncAndSeed = async()=> {
  ...
  await Promise.all([
    ...
    Role.create({ character: 'Carl Hanratty', movieId: catchMeIfYouCan.id, actorId: hanks.id}),
    Role.create({ character: 'Loraine', movieId: sully.id, actorId: linney.id}),
    Role.create({ character: 'Hannah Gill', movieId: truman.id, actorId: linney.id}),
